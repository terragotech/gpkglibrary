...
-(void) createUserTable: (GPKGUserTable *) table{
    ...
    for(int i = 0; i < [columns count]; i++){
        ...
        if (column.dataType == GPKG_DT_GEOMETRY) {
            if ([column isKindOfClass:[GPKGFeatureColumn class]]) {
                GPKGFeatureColumn *fcolumn = (GPKGFeatureColumn*)column;
                [sql appendFormat:@"\n %@ %@", column.name, [WKBGeometryTypes name:fcolumn.geometryType]];
            }else{
